...
class AppTests(unittest.TestCase):
    ...
    def test_dashboard_automatically_loads_and_chat_keeps_context(self):
        with tempfile.TemporaryDirectory() as root, patch.dict(os.environ, {
            ...
            data['metrics'] = [metric(), metric(2025, 120), dict(metric(2025, 245122), name='Net income')]
            data['sources'] = [{'id': f'r{year}-S1', 'report_id': f'r{year-2024}', 'page': 1,
                'source': f'FY{year}.pdf', 'report_type': 'Annual Report', 'fiscal_year': str(year),
                'fiscal_period': 'FY', 'content': f'Revenue {value}. FY{year}. USD millions.'}
                for year, value in [(2024, 100), (2025, 120)]]
            data['risks'] = [{'text': 'Competition is a disclosed risk.', 'source_id': 'r2024-S1', 'quote': 'Competition'}]
            answer = {'answer': 'Revenue grew [S1]', 'sources': [dict(data['sources'][0], id='S1')]}
            with patch('dashboard.load_dashboard', return_value=data) as analyze, patch('research.ask_equity_question', return_value=answer) as ask:
                app = AppTest.from_file(APP, default_timeout=30).run()
                self.assertFalse(app.exception)
