...
from ingest import ingest_pdf, list_companies, prepare_report, read_pdf
...
class PipelineTests(unittest.TestCase):
    ...
    def test_real_chroma_persistence_dedup_and_company_isolation(self, _):
        ...
        with patch(
            "ingest.extract_report_metadata", side_effect=AssertionError("Must not call API")
        ):
        ...
        store = Chroma(
            ...
            persist_directory=first["index_dir"],
    ...
    def test_unknown_metadata_rejected(self):
        with self.assertRaisesRegex(ValueError, "company"):
            ingest_pdf(
                pdf_bytes("Text"),
                "a.pdf",
                self.metadata.model_copy(update={"company_name": "Unknown"}),
                self.root,
            )
    ...
    def test_source_excerpts_on_same_page_are_combined(self):
        docs = [
            Document(page_content=text, metadata={"report_id": "a", "page": 1})
            for text in ["Revenue 100", "Income 20"]
        ]
    ...
    def test_invalid_citations_fail_closed(self):
        ...
        result = validate_answer(
            ResearchAnswer(answer="Revenue grew [S99]", source_ids=["S99"]), sources
        )
    ...
    def test_valid_citation_keeps_exact_report_page(self):
        result = validate_answer(
            ResearchAnswer(answer="Revenue 100 [S1]", source_ids=["S1"]),
    ...
    def test_retrieval_preserves_both_reports(self):
        (self.root / "metadata.json").write_text(
            json.dumps({"reports": [{"report_id": "a"}, {"report_id": "b"}]})
        )

        def search(query, k, filter):
            return [
                Document(
                    page_content=filter["report_id"],
                    metadata={"report_id": filter["report_id"], "page": 1},
                )
            ]

        with (
            patch("research.get_vector_store") as store,
            patch("research.generate_search_queries", return_value=["revenue"]),
        ):
    ...
    def test_follow_up_uses_history(self):
        with patch("research.get_llm") as client:
            client.return_value.invoke.return_value = SimpleNamespace(
                content="Why did revenue grow in FY2024?"
            )
            question = rewrite_follow_up_question(
                "Why?", [{"role": "user", "content": "Revenue FY2024"}]
            )
