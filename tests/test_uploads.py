...
def metadata(company="Microsoft Corporation", year="2024"):
    return ReportMetadata(
        company_name=company, fiscal_year=year, report_type="Annual Report", fiscal_period="FY"
    )
...
class UploadValidationTests(unittest.TestCase):
    ...
    def test_random_document_and_fabricated_evidence_rejected(self):
        detection = ReportDetection(
            **metadata().model_dump(),
            ...
            reason="Resume",
            issuer_quote="Microsoft",
            financial_quote="2024",
        )
    ...
    def test_verified_report_accepted(self):
        detection = ReportDetection(
            **metadata().model_dump(),
            ...
            reason="Annual results",
            issuer_quote="Microsoft Corporation",
            financial_quote="Revenue 200 million",
        )
        self.assertEqual(
            validate_detection(detection, "Microsoft Corporation\nRevenue 200 million").fiscal_year,
            "2024",
        )
    ...
    def test_manual_metadata_cannot_bypass_detection(self):
        with (
            ...
            patch(
                "ingest.extract_report_metadata", side_effect=ValueError("Not a financial report")
            ),
        ):
    ...
    def test_upload_opens_chat_and_removes_uploader(self):
        ...
        def detect(data):
            return (
                metadata("Microsoft", "2024")
                if data == a.data
                else metadata("Microsoft Corporation", "2025")
            )
    ...
    def test_comparison_requires_both_reports_evidence(self):
        with tempfile.TemporaryDirectory() as root:
            reports = [
                {**metadata().model_dump(), "report_id": rid, "filename": rid + ".pdf"}
                for rid in ["a", "b"]
            ]
            (Path(root) / "metadata.json").write_text(
                json.dumps({"company_name": "Microsoft", "reports": reports})
            )
            with patch(
                "research.ask_equity_question",
                return_value={"answer": "Growth was 10%", "sources": [{"report_id": "a"}]},
            ):
