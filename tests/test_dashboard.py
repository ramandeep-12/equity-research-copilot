...
def empty_dashboard(total=2):
    return {
        **{key: [] for key in ["insights", "metrics", "segments", "risks", "drivers", "sources"]},
        "analyzed": total,
        "total": total,
        "errors": [],
    }
...
def metric(year=2024, value=100):
    return dict(
        ...
        currency="USD",
        basis="GAAP",
        source_id=f"r{year}-S1",
        quote=f"Revenue {value}",
        period_quote=f"{year}",
        unit_quote="USD millions",
    )
...
class DashboardTests(unittest.TestCase):
    ...
    def test_growth_rate_change_uses_percentage_points(self):
        rows = [
            dict(metric(2024, 30), name="Azure revenue growth", unit="percent", currency="N/A"),
    ...
    def test_mixed_periods_currencies_and_bases_never_form_a_trend(self):
        rows = [
            metric(),
            dict(metric(2025, 120), duration_months=3, period="Q4"),
            dict(metric(2025, 120), currency="EUR"),
            dict(metric(2025, 120), basis="non-GAAP"),
        ]
    ...
    def test_unit_conversion_and_repeated_comparative_columns(self):
        rows = [metric(), dict(metric(2024, 0.1), unit="billions"), metric(2025, 120)]
    ...
    def test_unverified_figures_and_findings_are_dropped(self):
        ...
        brief = ReportBrief(
            insights=[Finding(text="Invented", source_id=source["id"], quote="Made up")],
            metrics=[DashboardMetric(**metric(value=200))],
            segments=[],
            risks=[],
            drivers=[],
        )
    ...
    def test_verified_metric_retains_source(self):
        source = {"id": "r2024-S1", "content": "Revenue 100. FY2024. USD millions."}
        brief = ReportBrief(
            insights=[], metrics=[DashboardMetric(**metric())], segments=[], risks=[], drivers=[]
        )
    ...
    def test_cache_only_analyzes_new_reports(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            brief = {
                key: []
                for key in ["insights", "metrics", "segments", "risks", "drivers", "sources"]
            }
    ...
    def test_partial_failure_preserves_success_and_can_retry(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch(
                "dashboard.analyze_report", side_effect=[brief, ValueError("No verified figures")]
            ):
