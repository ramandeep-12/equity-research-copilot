...
def validate_batch(pending, edits=None, active_company=None):
    ...
    if len(detected) != 1:
        ...
        raise ValueError("These reports belong to different companies: " + ", ".join(names) +
    ...
    if active_company and company_key(active_company) not in detected:
        raise ValueError(f"These reports do not belong to the active company, {active_company}. "
                         "Choose a new company library or upload reports from the active issuer.")
    if edits:
        for item, edit in zip(pending, edits):
            if edit is not None and company_key(edit['company_name']) != company_key(item['detail']['company_name']):
                raise ValueError("The company cannot be changed to a different issuer. Use the detected company identity.")
