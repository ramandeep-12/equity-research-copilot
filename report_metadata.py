...
class ReportMetadata(BaseModel):
    ...
    report_type: str = Field(
        description="Annual Report, Quarterly Report, Earnings Report, or Other"
    )
    fiscal_period: str = Field(default="Unknown", description="FY, Q1, Q2, Q3, Q4, or Unknown")


class ReportDetection(ReportMetadata):
    is_financial_report: bool = Field(
        description="True only for an issuer's annual, quarterly or earnings report with actual financial results"
    )
    ...
    financial_quote: str = Field(
        description="Exact continuous document excerpt disclosing financial results with numbers"
    )
...
def validate_detection(result, sample):
    ...
    if not result.is_financial_report or result.report_type not in {
        ...
        raise ValueError(
            "This document is not a supported company financial report. "
    ...
    if not company_key(result.company_name) or result.company_name.casefold() == "unknown":
        raise ValueError(
            "The report's issuer could not be verified. Upload a report with an identifiable company."
        )
    ...
    for quote in (result.issuer_quote, result.financial_quote):
        if not quote.strip() or normalize(quote) not in normalize(sample):
            raise ValueError(
                "The document's financial-report identity could not be verified from its text."
            )
...
def extract_report_metadata(file_bytes: bytes) -> ReportMetadata:
    ...
    result = (
        get_llm()
        .with_structured_output(ReportDetection)
        .invoke(
            [
                (
                    "system",
                    "Classify this document before extracting metadata. Accept ONLY an issuer's own "
