...
from research import build_sources, retrieve_evidence
...
class FinancialMetric(BaseModel):
    metric: str
    period: str = Field(
        description="Exact covered period including duration, e.g. FY2024 (12 months)"
    )
...
def quote_has_value(quote: str, value: float) -> bool:
    """Match printed numbers, including commas and accounting-style negatives."""
...
def extract_financial_metrics(index_dir, report_id, metric):
    sources = build_sources(
        retrieve_evidence(
            f"{metric} consolidated financial statements current fiscal reporting period currency units",
            index_dir,
            [report_id],
        )
    )
    ...
    result = (
        get_llm()
        .with_structured_output(MetricExtraction)
        .invoke(
            [
                (
                    "system",
                    "Extract the requested consolidated metric for ONLY the report's current covered "
