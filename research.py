...
MAX_QUESTION_LENGTH = 6000
MAX_HISTORY_MESSAGES = 6
MAX_SELECTED_REPORTS = 12
RETRIEVAL_CHUNKS_PER_QUERY = 4


class ResearchAnswer(BaseModel):
    answer: str = Field(
        description="Markdown answer with [S1], [S2] citations on every factual claim"
    )
    source_ids: list[str] = Field(description="Source labels actually cited, e.g. S1")
    compares_report_periods: bool = Field(
        default=False, description="True for comparisons between financial reporting periods"
    )


def get_vector_store(index_dir: str | Path) -> Chroma:
    """Open an existing company index with the configured embedding model."""
...
def rewrite_follow_up_question(question: str, chat_history: list[dict]) -> str:
    """Resolve references such as 'that year' using recent conversation turns."""
    if not chat_history:
        return question
    history = "\n".join(
        f"{m['role']}: {m['content'][:MAX_QUESTION_LENGTH]}"
    ...
    response = get_llm().invoke(
        [
            (
                "system",
                "Rewrite the latest question as a standalone financial research question. "
...
def generate_search_queries(question: str) -> list[str]:
    """Keep the original question and add up to three alternative search phrases."""
    response = get_llm().invoke(
        [
            (
                "system",
                "Return three short search queries, one per line, for evidence in financial reports. "
...
def rerank_documents(question, documents, top_k=5):
    ...
    response = get_llm().invoke(
        [
            (
                "system",
                f"Select up to {top_k} financial report chunks most relevant to the question. "
...
def retrieve_evidence(question, index_dir, report_ids=None):
    """Search and rerank each report independently to preserve comparison coverage."""
    ...
    if len(selected) > MAX_SELECTED_REPORTS:
    ...
    for report_id in selected:
        ...
        for query in queries:
            for doc in store.similarity_search(
                query, k=RETRIEVAL_CHUNKS_PER_QUERY, filter={"report_id": report_id}
            ):
...
def build_sources(documents):
    """Group retrieved chunks by report and page, then assign citation labels."""
    grouped = {}
...
def citation_label(source):
    return (
        f"{source.get('source', 'Report')} · {source.get('report_type', 'Report')} · "
        f"FY{source.get('fiscal_year', 'Unknown')} · "
        f"{source.get('fiscal_period', 'Unknown')} · PDF page {source['page']}"
    )


def validate_answer(response: ResearchAnswer, sources: list[dict]) -> dict:
    """Check citation IDs against retrieved sources; this does not prove each claim."""
    ...
    if not cited or cited != declared or not cited.issubset(lookup):
        return {
            "answer": "I could not produce an answer with verifiable source references. "
...
def ask_equity_question(question, index_dir, chat_history=None, report_ids=None):
    """Run the question-to-answer pipeline and return the cited evidence."""
    ...
    if not sources:
        return {
            "answer": "No evidence was found in the selected reports.",
            "sources": [],
            "used_sources": [],
        }
    context = "\n\n".join(
        f"[{s['id']}] REPORT_ID {s['report_id']} · {citation_label(s)}\n{s['content']}"
        for s in sources
    )
    response = (
        get_llm()
        ...
        .invoke(
            [
                (
                    "system",
                    "You are an equity research analyst. Use ONLY supplied evidence. "
    ...
    if (
        response.compares_report_periods
        and len({s["report_id"] for s in sources}) > 1
        and len({s["report_id"] for s in result["sources"]}) < 2
    ):
        result["answer"] = (
            "I could not verify a period comparison from both relevant reports. "
...
def compare_reports(topic, index_dir, baseline_id, comparison_id):
    ...
    if baseline_id == comparison_id:
        raise ValueError(
            "Select two distinct reports. Re-uploading the same document is not a comparison."
        )
    ...
    if any(
        company_key(r.get("company_name", manifest["company_name"])) != expected for r in selected
    ):
        raise ValueError(
            "These reports belong to different companies and cannot be compared in this workspace."
        )
    if not topic.strip():
        raise ValueError("Enter a comparison topic.")
    labels = [
        f"{role}: REPORT_ID {r['report_id']}; {r['filename']}; {r['report_type']}; "
    ...
    result = ask_equity_question(
        ...
        index_dir,
        report_ids=[baseline_id, comparison_id],
    )
    cited_reports = {s["report_id"] for s in result["sources"]}
    if not {baseline_id, comparison_id}.issubset(cited_reports):
        result["answer"] = (
            "A complete comparison could not be verified because the answer did not "
