...
VERSION = 1
CORE_METRICS = ["Revenue", "Operating income", "Net income", "Diluted EPS", "Operating cash flow", "Cloud revenue"]


class Finding(BaseModel):
    text: str = Field(description="One concise, plain-English factual insight, with its covered period")
    source_id: str
    quote: str = Field(description="Exact continuous excerpt supporting this finding")


class DashboardMetric(BaseModel):
    name: str = Field(description="Use Revenue, Operating income, Net income, Diluted EPS, Operating cash flow, Cloud revenue where applicable; otherwise an exact disclosed product growth metric, e.g. Azure revenue growth")
    scope: str = Field(description="Consolidated for company-wide metrics; otherwise the exact disclosed segment or product name")
    fiscal_year: int = Field(description="Fiscal year OF THE VALUE, not the report publication year")
    ...
    currency: str = Field(description="Explicit currency code; N/A for percentage metrics; Unknown if absent")
    basis: Literal["GAAP", "IFRS", "non-GAAP", "Unknown"]
    source_id: str
    quote: str = Field(description="Exact continuous excerpt containing the figure")
    period_quote: str = Field(description="Exact excerpt establishing the figure's fiscal period, e.g. the table headers")
    unit_quote: str = Field(description="Exact excerpt establishing units/currency; percentage excerpt for a growth metric")
...
def _contains(quote, content):
    normalize = lambda text: re.sub(r"\s+", " ", text).strip()
    return bool(quote.strip()) and normalize(quote) in normalize(content)


def validate_brief(brief, sources, report):
    """Only publish findings with real quotes and metrics with real signed figures."""
    lookup = {s['id']: s for s in sources}
    validated = {name: [] for name in ['insights', 'metrics', 'segments', 'risks', 'drivers']}
    for section in ['insights', 'segments', 'risks', 'drivers']:
        for finding in getattr(brief, section)[:6]:
    ...
    seen = set()
    for metric in brief.metrics[:40]:
        source = lookup.get(metric.source_id)
        key = (metric.name, metric.scope, metric.fiscal_year, metric.period,
               metric.duration_months, metric.unit, metric.currency, metric.basis)
        expected_duration = {'FY': 12, 'Q1': 3, 'Q2': 3, 'Q3': 3, 'Q4': 3, 'H1': 6, '9M': 9}.get(metric.period)
        if (source and key not in seen and math.isfinite(metric.value)
...
def analyze_report(index_dir, report):
    ...
    for query in queries:
        for doc in store.similarity_search(query, k=4, filter={'report_id': report['report_id']}):
            unique[(doc.metadata.get('page'), doc.metadata.get('start_index'), doc.page_content)] = doc
    ...
    brief = get_llm().with_structured_output(ReportBrief).invoke([
        ('system', "Produce an equity research dashboard using ONLY the supplied report evidence. "
    ...
    if not any(result[name] for name in ['insights', 'metrics', 'segments', 'risks', 'drivers']):
        raise ValueError("No dashboard findings could be verified against this report's evidence.")
    return result


def dashboard_signature(reports):
    data = {'version': VERSION, 'model': os.getenv('OPENAI_MODEL', 'gpt-4.1-mini'),
...
def load_dashboard(index_dir, progress=None, report_ids=None):
    ...
    for i, report in enumerate(sorted(reports, key=lambda r: (r['fiscal_year'], r['report_id']))):
        ...
        try:
            if report.get('report_type') not in {'Annual Report', 'Quarterly Report', 'Earnings Report', '10-K', '10-Q'}:
                raise ValueError("This legacy document is not a supported financial report. Upload a validated company report.")
            ...
            errors.append({'report': report['filename'], 'message': str(exc) if isinstance(exc, ValueError)
                           else 'Analysis failed. Check API access and retry.'})
    result = {section: [item for brief in reversed(briefs) for item in brief[section]]
...
SCALES = {'ones': 1, 'thousands': 1e3, 'millions': 1e6, 'billions': 1e9,
          'percent': 1, 'per share': 1}
...
def series_groups(rows):
    ...
    for row in rows:
        ...
        key = (row['name'], row['scope'], row['period'], row['duration_months'], row['currency'], row['basis'], unit_kind)
    ...
    for key, years in groups.items():
        ...
        for year, candidates in sorted(years.items()):
            ...
            if len(values) != 1:
                ...
                continue
            points.append({**candidates[0], 'normalized_value': next(iter(values)),
...
def automatic_changes(groups):
    ...
    for group in groups:
        ...
        if after['unit'] == 'percent':
            ...
            change = f"{after['currency']} {abs(delta):,.2f}" + (' per share' if after['unit'] == 'per share' else '')
        text = (f"{prefix} {direction}" + (f" by {change}" if delta else '') +
                f" from {before['period_label']} to {after['period_label']}.")
        changes.append({'text': text, 'source_ids': list(dict.fromkeys([before['source_id'], after['source_id']]))})
    return changes
